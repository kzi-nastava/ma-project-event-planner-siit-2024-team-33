...
public class ProviderSignupViewModel extends AndroidViewModel {

    private final AuthenticationService authService;

    public ProviderSignupViewModel(@NonNull Application application) {
        super(application);
    ...
    public final MutableLiveData<List<String>> providerPictures = new MutableLiveData<>(new ArrayList<>());
    public final MutableLiveData<String> providerName = new MutableLiveData<>();
    ...
    public final MutableLiveData<String> providerNameError = new MutableLiveData<>();
    public final MutableLiveData<String> descriptionError = new MutableLiveData<>();
    ...
    public boolean isStepValid(int step) {
        ...
        switch (step) {
            ...
            case 2: //mandatory data
                ...
                String pn = providerName.getValue() != null ? providerName.getValue().trim() : "";
                if (pn.isEmpty() || !pn.matches("^[a-zA-Z0-9]{1,20}$")) {
                    providerNameError.setValue("Provider Name required (1-20 letters/numbers)");
                    valid = false;
                } else providerNameError.setValue(null);
                ...
                if (desc.isEmpty() || !desc.matches("^.{20,}$")) {
                    descriptionError.setValue("Description required (at least 20 characters)");
                    valid = false;
                } else descriptionError.setValue(null);
    ...
    public void submitSignup() {
        ...
        registerUser.setProviderName(providerName.getValue());
        registerUser.setDescription(description.getValue());
        registerUser.setResidency(residency.getValue());
        registerUser.setPhoneNumber(phone.getValue());
        registerUser.setPicture(profilePicture.getValue());
        registerUser.setPictures(providerPictures.getValue());
        registerUser.setRole("PROVIDER_ROLE");
    ...
    //put new provider pictures with new selection
    public void setProviderPictures(List<Bitmap> bitmaps, List<String> mimeTypes) {
        List<String> newList = new ArrayList<>();

        for (int i = 0; i < bitmaps.size() && i < mimeTypes.size() && newList.size() < 8; i++) {
            ...
            if (bitmap != null && mimeType != null) {
                ...
                if (isProfilePictureValid(base64)) {
                    ...
                    signupError.setValue("Invalid picture (unsupported type or too large)");
                }
            }
        }

        providerPictures.setValue(newList);
    }
}